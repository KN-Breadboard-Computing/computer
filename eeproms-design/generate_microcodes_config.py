...
microcodes.add('LOAD_ZP_MEM[MAR]_TO_TL').mem_to_bus(zero_page=True).reg_from_bus(REG_TL)
microcodes.add('LOAD_ZP_MEM[MAR]_TO_TH').mem_to_bus(zero_page=True).reg_from_bus(REG_TH)
...
microcodes.add('PC++_RESET_ISR_FLAG').pc_inc().set_int_enable()
